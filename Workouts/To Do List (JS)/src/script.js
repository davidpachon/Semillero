...
let descripcionTareaSeleccionada=document.querySelector('.descripcion');
...
function asignarPomodoro(){    
    ...
    descripcionTareaSeleccionada.innerHTML=``; 
        ...
        for(let i=0; i< tareas.length; i++){
            for(let i=0; i< todasTareas.length; i++){
                if(titulo==tareas[i].titulo){
                    ...
                    descripcionTareaSeleccionada.innerHTML=`${tareas[i].descripcion}`; 
...
function mantenerTareaSeleccionada(){
    ...
    for(let i=0; i< tareas.length; i++){ 
        if(localStorage.getItem("CurrentTitle") != null ){
            ...
            tareas[i].titulo == localStorage.getItem("CurrentTitle") ? (tareas[i].pomodoros == 1 ? numeroPomodoros.innerHTML=`${tareas[i].pomodoros} pomodoro`: numeroPomodoros.innerHTML=`${tareas[i].pomodoros} pomodoros`,todasTareas[i].style.background="#191f70",descripcionTareaSeleccionada.innerHTML=`${tareas[i].descripcion}`): null;
        }        
    }          
}
mantenerTareaSeleccionada()
